...
SELECTION-SCREEN BEGIN OF SCREEN 200.
  ...
  SELECTION-SCREEN BEGIN OF BLOCK ticket WITH FRAME TITLE TEXT-001.
    ...
    PARAMETERS:    p_stat    TYPE zalsu_db_ticket-status.
...
FORM selection.
  ...
  p_stat = ls_ticket2-zalsu_db_ticket-status.
...
FORM eingabe.
  ...
  ELSEIF p_tech_z IS NOT INITIAL.    "Tritt ein wenn zum Techniker weitergeleitet => Beim Techniker
    ...
    UPDATE zalsu_db_ticket SET status = 'IN BEARBEITUNG' , technikerid = @lv_tech, kundeninfo = 'X'
    WHERE ticketid = @p_tickid.
    DATA(lv_message2) = |Das Ticket { p_tickid } wurde an { p_tech_z-nachname } weitergeleitet. { p_vname } { p_nname } wurde per E-Mail informiert.|.
