*&---------------------------------------------------------------------*
*& Report zmaf_report_serviceticket_app
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zmaf_report_serviceticket_app.
...
SELECTION-SCREEN BEGIN OF BLOCK sto WITH FRAME TITLE TEXT-002.
  ...
  PARAMETERS: p_tech TYPE znachname AS LISTBOX VISIBLE LENGTH 16. "Listbox Techniker
...
SELECTION-SCREEN BEGIN OF LINE.
  ...
  SELECTION-SCREEN: PUSHBUTTON 83(25) exec3 USER-COMMAND exec3.
...
INITIALIZATION.
  ...
  exec3     = ICON_CASHING_UP  && 'Analyse (Datum-->)'.          "EXEC3
  dat       = icon_date        && 'von heute'.                   "SHOW1
  quick     = ICON_LATE_LINK   && 'älteste offene Störung'.      "FC9
  anzahl    = icon_calculation && 'Anzahl unerledigte'.          "FC10
...
* Abruf Anzahl unerledigter Vorgänge
    ...
    WHEN 'FC10'. "Button: Anzahl unerledigte
