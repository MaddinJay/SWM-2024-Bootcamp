CLASS zst_class_verbrauch DEFINITION
*&---------------------------------------------------------------------*
*& Include z_efms_cl_treibstoffverbrauch
*&---------------------------------------------------------------------*
PUBLIC.
  PUBLIC SECTION.
...
*RETURNING VALUE(rv_leerflug) type saplane-consum.   "kann bei Methodenaufruf direkt angegeben werden.
  ...
  PRIVATE SECTION.
    ...
    class-data rv_kgbedarf type i.
    Class-data rv_pbedarf type i.
    CONSTANTS mc_person TYPE i VALUE 102. " 77 kg (Standardperson) + 25 kg Gepäck bzw. Handgepäck

ENDCLASS.

CLASS zst_class_verbrauch IMPLEMENTATION.
...
*******************************
    ...
    rv_flug = ( ls_struktur-consum / ls_struktur-weight *  ( rv_gewicht_passagiere + ls_struktur-weight ) ).
...
Write: rv_seatsocc_gesamt.
Write: rv_gewicht_passagiere.
Write: ls_struktur-weight.
