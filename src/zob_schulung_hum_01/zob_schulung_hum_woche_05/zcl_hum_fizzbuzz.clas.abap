CLASS zcl_hum_fizzbuzz DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_hum_fizzbuzz IMPLEMENTATION.

  METHOD convert_number.
    DATA lv_number(3) TYPE n.
    lv_number = iv_number.
    rv_number = COND #(  WHEN lv_number = 1 THEN |1|
                         WHEN lv_number MOD 3 EQ 0 AND iv_number MOD 5 EQ 0  THEN |{ fizz }{ buzz }|
                         WHEN lv_number MOD 3 EQ 0 THEN |{ fizz }|
                         WHEN lv_number MOD 5 EQ 0 THEN |{ buzz }|
                         WHEN lv_number CA '3' AND lv_number CA '5' THEN |{ buzz }{ fizz }|
                         WHEN lv_number CA '3' THEN |{ fizz }|
                         WHEN lv_number CA '5' THEN |{ buzz }|
                         ELSE lv_number ).
