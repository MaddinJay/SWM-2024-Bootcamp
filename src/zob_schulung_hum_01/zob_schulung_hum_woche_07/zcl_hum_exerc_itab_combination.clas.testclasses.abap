CLASS ltcl_hum_exerc_itab_combinati DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PUBLIC SECTION.
    DATA cut TYPE REF TO zcl_hum_exerc_itab_combination.
  ...
  PRIVATE SECTION.
    METHODS setup.
ENDCLASS.
CLASS ltcl_hum_exerc_itab_combinati IMPLEMENTATION.
  METHOD setup.
    cut = NEW zcl_hum_exerc_itab_combination( ).
  ENDMETHOD.
  METHOD test_empty_input.
    cl_abap_unit_assert=>assert_equals(
      act = cut->perform_combination( alphas = VALUE #( )
                                      nums = VALUE #( ) )
      exp = VALUE zcl_hum_exerc_itab_combination=>combined_data( ) ).
  ENDMETHOD.
  METHOD test_single_row.
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = VALUE zcl_hum_exerc_itab_combination=>combined_data( ( colx = 'A1' coly = 'B2' colz = 'C3' ) ) ).
  ENDMETHOD.
