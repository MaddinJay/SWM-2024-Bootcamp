CLASS ltcl_hum_exercism_itab_basics DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.

    DATA cut TYPE REF TO zcl_hum_exercism_itab_basics.
    METHODS setup.
...
CLASS ltcl_hum_exercism_itab_basics IMPLEMENTATION.
  METHOD setup.
    cut = NEW zcl_hum_exercism_itab_basics( ).
  ENDMETHOD.

  METHOD test_fill_itab.
    DATA(actual) = cut->fill_itab( ).
    cl_abap_unit_assert=>assert_not_initial( actual ).
    DATA(expected_values) = VALUE zcl_hum_exercism_itab_basics=>itab_data_type(
  ...
  METHOD test_sort_itab_empty.
    cl_abap_unit_assert=>assert_equals(
      ...
      exp = VALUE zcl_hum_exercism_itab_basics=>itab_data_type( ) ).
  ENDMETHOD.
