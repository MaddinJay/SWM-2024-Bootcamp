CLASS ltcl_hum_exerc_grains DEFINITION FOR TESTING DURATION SHORT RISK LEVEL HARMLESS FINAL.
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_hum_exerc_grains.
    METHODS setup.
...
ENDCLASS.
CLASS ltcl_hum_exerc_grains IMPLEMENTATION.
  METHOD setup.
    cut = NEW zcl_hum_exerc_grains( ).
  ENDMETHOD.
  "grains on square 1
  ...
  ENDMETHOD.
  "grains on square 64
  METHOD test_square_64.   "Lösung falsch, 8 Körner zu wenig laut Ergebnis :(
    cl_abap_unit_assert=>assert_equals(
      act = cut->square( 64 )
