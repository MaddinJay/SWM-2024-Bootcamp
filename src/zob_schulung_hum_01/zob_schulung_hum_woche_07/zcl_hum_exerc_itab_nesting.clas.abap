CLASS zcl_hum_exerc_itab_nesting DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_hum_exerc_itab_nesting IMPLEMENTATION.
  METHOD perform_nesting.
  LOOP AT artists REFERENCE INTO DATA(artist).
      APPEND INITIAL LINE TO nested_data REFERENCE INTO DATA(l_data).
      l_data->artist_id   = artist->artist_id.
      l_data->artist_name = artist->artist_name.
      LOOP AT albums REFERENCE INTO DATA(album)
            WHERE artist_id = artist->artist_id.
        APPEND INITIAL LINE TO l_data->albums REFERENCE INTO DATA(l_album).
        l_album->album_id   = album->album_id.
        l_album->album_name = album->album_name.
        LOOP AT songs REFERENCE INTO DATA(song)
            WHERE artist_id = album->artist_id
              AND album_id = album->album_id.
          APPEND INITIAL LINE TO l_album->songs REFERENCE INTO DATA(l_song).
          l_song->song_id    = song->song_id.
          l_song->song_name  = song->song_name.
        ENDLOOP.
      ENDLOOP.
    ENDLOOP.
    ENDMETHOD.
